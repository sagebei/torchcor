"""Published participant results for the Arostica et al. benchmark.

Source
------
``Reidmen/cardiac_benchmark_toolkit``, ``results/data`` (the repository the
paper points to for its comparison curves), downloaded 2026-09-10 into
``reference/``.  Eleven participating codes, each sampled at 10 ms over the
one-second beat, giving the ``T = 101`` datapoints of equation (21).

The discrepancy measure is the paper's RED, reimplemented from that
repository's own ``results/figures.py`` so that our number means exactly what
the published ones do: the time average of the displacement error relative to
...
The participants' own RED values, computed the same way, are the yardstick:
being no further from their mean than they are from each other is what
...
#: Benchmark case -> the filename stem the repository stores it under.
CASES = {"active": "monoventricular_nonblinded_step_0A",
         "passive": "monoventricular_nonblinded_step_0B",
         "both": "monoventricular_nonblinded_step_1",
         "A": "monoventricular_blinded_A",
         "B": "monoventricular_blinded_B",
         "C": "monoventricular_blinded_C"}
PROBES = ("p0", "p1")
#: The ten datasets the paper's own comparison uses, in its table order.  The
#: repository also ships a plain ``simvascular`` file that the published
#: figures leave out; including it would change the mean every RED is measured
#: against, so the list is taken from ``figures.py`` rather than from the
#: directory.
TEAMS = ("carpentry", "ambit", "4C", "simula", "chimera", "cheart", "lifex",
         "simvascular_p1p1", "simvascular_p2", "comsol")
...
def participants(case):
    ...
    out = {}
    for team in TEAMS:
        found = (glob.glob(str(HERE/f"{CASES[case]}_group_{team}.*"))
                 or glob.glob(str(HERE/f"{CASES[case]}_group_{team.lower()}.*")))
        if not found:
            continue
        path, name = found[0], team
        raw = pickle.load(open(path, "rb"))
        out[name] = {"time": np.asarray(raw["time"], dtype=float),
